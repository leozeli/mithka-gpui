...
- **联系人**：分组栏的 Contacts（`user-group`）。发送 `getContacts`，并用 `updateUser` 更新（`is_contact`、`usernames.active_usernames`、小头像 `profile_photo.small`）。一行是头像、名字和 `@username`（没有用户名就只显示名字）。点击会 `createPrivateChat`（`force` 为 false）再打开私聊。还没有按手机号添加联系人的表单。点文件夹或 All chats 会离开联系人列表。
