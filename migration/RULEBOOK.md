...
- A human kicks the next manifest row. The next module is richer messaging (M14).
  Settings and notifications (M13) are `done`. Profile (M12) is `done`.
...
| Telegram folder vs local folder group | A Telegram folder is a TDLib chat folder (`FolderItem`, id from `updateChatFolders`). A local folder group (design name LocalFolderGroup; Rust type `LocalGroup` in `gpui/src/groups.rs`) nests those folders. It is a name plus `NestedFolder::Main` (`{ "kind": "main" }`) and `NestedFolder::Folder { id }` (`{ "kind": "folder", "id": N }`). Schema version 2. A version 1 file that stored chat ids is rewritten on load and those ids are dropped. Groups do not filter by chat id. All chats and Contacts hide the folders column (groups rail + list + open chat). The folders column is shown only while a local group is selected, and then only that group's nested folders plus Attach. Attach adds a folder to the group. Remove takes it out. All chats clears the group selection, hides that column, and loads the main list. Contacts keeps rows in the list column and does not paint folder rows. Subscriptions still uses the middle column for feed sources. | `gpui/src/groups.rs` |
...
| Profile page | Replaces the conversation pane until Back. It does not add a column. Open it from the chat header (title, avatar, or Profile) for the open chat’s peer, or from Profile on a Contacts row. The rest of that row still opens the private chat. Peers are a private user, a secret chat, a basic group, or a channel/supergroup. TDLib calls are `getUser` / `getUserFullInfo`, `getBasicGroup` / `getBasicGroupFullInfo`, `getSupergroup` / `getSupergroupFullInfo`, and `getChat` for the open chat. Phone is shown only when `phone_number` is non-empty. No gift store, QR, media gallery, block, or edit-profile form. | `src/shell.rs`; `gpui/src/main.rs`; S14 |
| Settings and notifications | Settings (`cog-6-tooth`, groups column footer) replaces the conversation pane until Back. It does not add a column. Sections are Notifications, Appearance, Account, Privacy, Data & storage, and About. Only Notifications is live. The others show “Coming soon”. Mute is stored only in TDLib: `setChatNotificationSettings` for the open chat (header Mute/Unmute and the Notifications detail) and `getScopeNotificationSettings` / `setScopeNotificationSettings` for private, group, and channel defaults (`mute_for`, `show_preview`). A mute longer than 366 days is forever. A chat with `use_default_mute_for` follows its scope. Desktop push is not sent. A failure stays on the status line and does not close the session. | `src/shell.rs`; `src/notify.rs`; `gpui/src/main.rs`; S15, S16 |
